...
const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || 'https://www.langfordstaffing.com'
...
export async function generateMetadata(
  ...
  if (!role) {
    return {
      title: 'Role Not Found | Langford Staffing',
      robots: { index: false, follow: false },
    }
  }
  const title = `${role.title}, ${role.locationDisplay} | Langford Staffing Careers`
  const description = `${role.title} (${role.type}) at Langford Staffing in ${role.locationDisplay}. ${role.summary ? role.summary.slice(0, 150) : role.title}`
  return {
    title,
    description,
...
export default async function CareerRolePage({ params }: RouteParams) {
  ...
  const shareSubject = `${role.title} at Langford Staffing`
  ...
  return (
    <main className="bg-brand-paper">
      ...
      <section className="bg-brand-navy text-white grain">
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14 lg:px-10 lg:py-16">
          <Link
            href="/careers/#positions"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-white/85 transition-colors hover:text-white no-underline"
          >
          ...
          <div className="mt-8 grid grid-cols-1 gap-10 lg:gap-12">
            <div className="min-w-0">
              <h1 className="font-display text-4xl font-bold leading-[1.05] tracking-tight text-white sm:text-5xl md:text-6xl lg:text-[4.25rem]">
                {role.title}
              </h1>
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[14rem_1fr] lg:gap-16">
            <aside className="lg:pt-2 lg:sticky lg:top-24 lg:self-start">
              ...
              <ul className="mt-8 hidden space-y-3 text-xs text-brand-ink/70 lg:block">
                <li className="flex items-start gap-2">
                  <Briefcase
                    className="mt-0.5 h-3.5 w-3.5 shrink-0 text-brand-saffron"
                    aria-hidden="true"
                  />
                  <span>{role.type}</span>
                </li>
              ...
              <div className="mt-8 hidden lg:block rounded-xl bg-white p-4 border border-brand-line/5">
                <h3 className="text-lg font-bold text-brand-navy mb-2 tracking-wider">Important Location Requirement:</h3>
                <div className="text-sm text-brand-ink space-y-3 tracking-wider">
                  <p>
                  ...
                  <p className="text-sm font-semibold text-brand-ink tracking-wider">
            ...
            <article className="min-w-0">
              {role.htmlDescription ? (
                ...
                <>
                  <Block title="Job Description Summary">
                    <p className="text-[15px] leading-[1.8] text-brand-ink/90">
                      {role.summary}
                    </p>
                  </Block>

                  <Block title="Job Description">
                    <h4 className="text-[12px] font-bold uppercase tracking-[0.18em] text-brand-navy">
                      Key Responsibilities
                    </h4>
                  ...
                  <Block title="Additional Information">
                    ...
                    <p className="mt-3 text-[15px] leading-[1.8] text-brand-ink/90">
              ...
              <div className="mt-12 flex flex-wrap gap-3 border-t border-brand-line/5 pt-10">
                <ApplyButton role={role.title} jobId={role.jobId} jobOpeningId={role.jobOpeningId} workType={role.workType} />
                <Link
                  href="/careers/#positions"
                  className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full border border-brand-navy/20 bg-white px-6 py-3 text-sm font-bold text-brand-navy transition-colors hover:border-brand-navy/40 hover:bg-white no-underline"
                >
                  <ArrowLeft className="h-4 w-4" aria-hidden="true" />
                  See all open roles
                </Link>
  ...
  return (
    <ul className="mt-3 space-y-2">
      {items.map((item, i) => (
        <li
          key={i}
          className="flex items-start gap-2.5 text-[15px] leading-[1.7] text-brand-ink/90"
        >
          <span
            aria-hidden="true"
            className="mt-2 inline-block size-1.5 shrink-0 rounded-full bg-brand-saffron"
          />
          <span>{item}</span>
        </li>
      ))}
