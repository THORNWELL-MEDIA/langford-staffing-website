...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: ["careers@langfordstaffing.com"],
      replyTo: email,
      subject: `New Application: ${role} — ${firstName} ${lastName}`,
      html: `
        <h2>New Job Application — Langford Staffing</h2>
        ...
        <p style="margin-top:24px;color:#666;font-size:12px">Sent from langfordstaffing.com/careers</p>
      `,
    });
