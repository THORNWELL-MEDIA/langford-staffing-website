            ...
            switch (_a.label) {
                ...
                case 3:
                    ...
                    return [2 /*return*/, apiJobs.map(function (job) {
                        ...
                        var styleStripRegex = /(font-family|font-size|color|background-color|background|line-height|padding|padding-inline-start|padding-left|padding-right|margin|margin-left|margin-right|list-style|list-style-type|list-style-position|text-indent)\s*:[^;]+;?/gi;
                        ...
                        rawHtml = rawHtml.replace(/<li[^>]*>([\s\S]*?)<\/li>/gi, function (match, inner) {
                            var cleanedInner = inner.replace(/<\/?h[1-6][^>]*>/gi, function (tag) { return tag.startsWith('</') ? '</b>' : '<b>'; });
                        ...
                        var workTypeLower = workTypeRaw.toLowerCase();
                        var jobTypeLower = jobTypeRaw.toLowerCase();
                        var employmentTypeDisplay = workTypeRaw || 'Full-Time';
                        if (jobTypeRaw) {
                            if (workTypeRaw) {
                                if (!workTypeLower.includes(jobTypeLower)) {
                                    employmentTypeDisplay = "".concat(workTypeRaw, " \u00B7 ").concat(jobTypeRaw);
                                }
                            }
                            else {
                        ...
                        var workArrangementDisplay = jobTypeRaw;
                        var locParts = [];
                        if (job.City)
    ...
    return countryOrder.map(function (countryKey) {
        ...
        return {
            ...
            regions: country.regionOrder.map(function (regionKey) {
                ...
                return {
                    region: regionKey,
                    cities: region.cityOrder.map(function (cityKey) {
                        return ({
                            city: cityKey,
